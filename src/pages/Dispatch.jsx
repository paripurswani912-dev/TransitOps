...
export default function Dispatch() {
  ...
  return (
    <div className="space-y-6">
      {/* Page Title */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight font-sans">Trip Dispatcher</h2>
        <p className="text-sm text-gray-500 mt-1">Dispatch verified allocations and track delivery life-boards in real-time.</p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-200 shadow-sm">
          ...
          <p className="text-sm font-semibold tracking-wider text-gray-400 font-mono uppercase">Syncing Operations Hub...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <section className="lg:col-span-5 bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
            <div className="border-b border-gray-100 pb-4">
              <h3 className="text-lg font-bold text-gray-900 font-sans">Create Trip</h3>
              <p className="text-xs text-gray-400 mt-1 font-mono">Stage: Preparation Tunnel</p>
            </div>
                ...
                {[
                  ...
                  return (
                    <div key={step.key} className="flex flex-col items-center relative z-10">
                      <div className={`h-5 w-5 rounded-full flex items-center justify-center border-2 text-[9px] font-bold font-mono transition-all duration-300 ${
                        isActive 
              ...
              {isCapacityExceeded && (
                <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs text-red-800 flex items-start gap-3 animate-shake">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-600 leading-normal mt-1">
                      Vehicle Capacity <span className="font-mono font-bold">{selectedVehicleObj.capacity.toLocaleString()} kg</span> / Cargo Weight <span className="font-mono font-bold">{cargoWeightNum.toLocaleString()} kg</span> — Capacity exceeded by <span className="font-mono font-bold">{capacityDifference.toLocaleString()} kg</span> → dispatch blocked
                    </p>
                  </div>
                </div>
              )}
          ...
          <section className="lg:col-span-7 space-y-4">
            <div className="flex items-center justify-between border-b border-gray-200 pb-3 bg-white p-4 rounded-t-2xl border">
              <div>
                <h3 className="text-lg font-bold text-gray-900 font-sans">Live Board</h3>
                <p className="text-xs text-gray-500 font-mono mt-0.5">Real-Time Operational Dispatch Trackers</p>
              </div>
              <span className="text-[10px] font-mono bg-blue-50 border border-blue-200 text-blue-800 px-2 py-0.5 rounded-md uppercase font-bold animate-pulse">
                live feed sync
              </span>
            </div>
            ...
            {trips.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-200 shadow-sm text-center">
                ...
                <p className="text-sm font-semibold text-gray-400 font-mono uppercase">No active trip logs found</p>
              </div>
            ) : (
              <div className="space-y-4 max-h-[660px] overflow-y-auto pr-1">
                {trips.map((trip) => {
                  ...
                  return (
                    <div 
                      key={trip.id}
                      className={`border bg-white p-5 rounded-2xl transition-all shadow-sm ${
                        trip.status === 'Dispatched' ? 'border-blue-200 hover:border-blue-400' :
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between gap-4">
                        <span className="font-mono font-bold text-gray-900 bg-gray-100 border px-2.5 py-0.5 rounded-lg text-xs">
                          {trip.id.length > 8 ? `TR-${trip.id.slice(0, 5).toUpperCase()}` : trip.id}
                        </span>

                        <div className="flex items-center space-x-2">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                            trip.status === 'Dispatched' ? 'bg-blue-50 text-blue-800 border border-blue-200' :
                            trip.status === 'Completed' ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' :
                            trip.status === 'Cancelled' ? 'bg-rose-50 text-rose-800 border border-rose-200' :
                            'bg-gray-100 text-gray-800 border border-gray-200'
                          }`}>
                            <span className={`h-1.5 w-1.5 rounded-full ${
                              trip.status === 'Dispatched' ? 'bg-blue-500 animate-pulse' :
                      ...
                      {/* Route Map Section */}
                      <div className="mt-3.5 flex items-center gap-3 text-sm text-gray-800">
                      ...
                      {/* Resource details */}
                      <div className="grid grid-cols-2 gap-4 mt-4 text-xs font-sans text-gray-500 border-t border-gray-100 pt-3">
                        <div>
                          <span className="block font-bold text-gray-400 uppercase tracking-wider font-mono text-[9px]">Vehicle allocated</span>
                          <span className="block mt-0.5 text-gray-700 font-mono">{trip.vehicleId || 'Awaiting Vehicle'}</span>
                          {vehicleObj && (
                            <span className="text-[10px] text-gray-400 block mt-0.5">({vehicleObj.name})</span>
                          )}
                        </div>
                        <div>
                          <span className="block font-bold text-gray-400 uppercase tracking-wider font-mono text-[9px]">Driver assigned</span>
                          <span className="block mt-0.5 text-gray-700">{driverObj ? driverObj.name : 'Awaiting Driver'}</span>
                          {driverObj && (
                            <span className="text-[10px] text-gray-400 block mt-0.5">Score: {driverObj.safetyScore}/100</span>
                          )}
                        </div>
                      </div>

                      {/* Card Footer notes / ETA */}
                      <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3 text-[11px] font-mono text-gray-400">
                        <span>Cargo Weight: <strong className="text-gray-600">{trip.cargoWeight.toLocaleString()} kg</strong></span>
                        <span>Distance: <strong className="text-gray-600">{trip.plannedDistance.toLocaleString()} km</strong></span>
                      </div>

                      {/* Note / ETA block */}
                      <div className="mt-3 text-xs bg-gray-50 border p-2.5 rounded-xl font-mono text-gray-600 flex items-center gap-2">
                      ...
                      {trip.status === 'Dispatched' && completingTripId !== trip.id && !isReadOnly && (
                        <div className="mt-4 flex items-center gap-2 border-t border-gray-100 pt-3 font-sans">
                          <button
                            ...
                            className="flex-1 py-2 bg-white border border-rose-200 text-rose-700 text-xs font-semibold rounded-xl hover:bg-rose-50/50 hover:border-rose-300 transition-colors cursor-pointer text-center"
                          >
                            Cancel Dispatch
                          </button>
                      ...
                      {completingTripId === trip.id && (
                        <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-xl space-y-3 font-sans animate-slide-in">
                          <div className="flex justify-between items-center border-b border-gray-200 pb-1.5 mb-1">
                            <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider font-mono">Complete Trip Parameters</h4>
                            <span className="text-[10px] text-emerald-600 font-mono font-semibold uppercase">Trip Allocation release</span>
                          </div>

                          {inlineError && (
                            <div className="rounded-lg border border-red-200 bg-red-50/60 p-2.5 text-[10px] text-red-800 flex items-start gap-1.5 animate-shake">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider font-mono mb-1">Final Odometer (km)</label>
                              <div className="relative">
                                ...
                                <input
                                  ...
                                  className="w-full rounded-lg border border-gray-200 bg-white py-1.5 pl-8 pr-2 text-xs outline-none focus:border-amber-500 font-mono"
                                />
                              </div>
                            </div>
                          ...
                          <div className="flex gap-2 pt-2 border-t border-gray-200 mt-2">
                            <button
                              ...
                              className="px-3 py-1.5 border border-gray-300 rounded-lg text-xs font-semibold text-gray-700 bg-white hover:bg-gray-50 transition-colors cursor-pointer"
                            >
                              Cancel
                            </button>
                            ...
                            <button
                              ...
                              className="flex-1 inline-flex items-center justify-center gap-1.5 py-1.5 bg-emerald-500 text-white rounded-lg text-xs font-bold hover:bg-emerald-600 shadow-sm transition-colors cursor-pointer disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed"
                            >
                              {isCompleting ? (
                                <>
            ...
            <div className="flex items-start gap-2 bg-gray-50 border border-gray-200 p-3 rounded-2xl text-[11px] font-mono text-gray-400">
              <Info className="h-4 w-4 text-gray-400 shrink-0 mt-0.5" />
              <span>
                <strong>System Logic Map:</strong> On Complete: odometer → fuel log → expenses → Vehicle & Driver Available
              </span>
            </div>
          </section>
